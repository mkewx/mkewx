...
CHAPTER2_ELASTIC_LAB = 1
CHAPTER2_ADVANCED_ELASTIC_LAB = 1
        ENDIF
...
STAGE3_SPAWN_X         = 48
STAGE3_SPAWN_Y         = 40
STAGE3_DISH_A_LEFT     = 75
STAGE3_DISH_B_LEFT     = 117
STAGE3_DISH_C_LEFT     = 75
STAGE3_DISH_A_TOP      = 27
STAGE3_DISH_B_TOP      = 81
STAGE3_DISH_C_TOP      = 131
STAGE3_BEACON_AX       = 79
STAGE3_BEACON_AY       = 34
STAGE3_BEACON_BX       = 121
STAGE3_BEACON_BY       = 88
STAGE3_BEACON_CX       = 79
...
stageOffset     ds 1         ; transient display color phase; never persistent
...
.collisionReady:
        ...
        and #3
        sta bestBeacon
        IFCONST CHAPTER2_ADVANCED_ELASTIC_LAB
        ; Physics has finished using stageOffset as candidate X. Convert it
        ; into the display-only elastic color phase before returning to the
        ; timer-controlled VBLANK caller.
        lda frameCounter
        lsr
        lsr
        lsr
...
ResolveFreeTerrainCollision:
        ...
        lda tomXHi
        sta stageOffset         ; transient candidate X; renderer overwrites it
        lda tomYHi
        sta bestDistance
        lda #0
...
.freeVertical:
        ...
        sta temp
        jsr IsChapter2ElasticContact
        bcc .ordinaryVerticalBounce
        lda temp
        bmi .elasticBounceDown
        lda #$FE
        sta velYHi
        lda #$80
        sta velYLo
...
.freeCollisionReady:
        ...
        IFCONST CHAPTER2_ADVANCED_ELASTIC_LAB
; Zone 2-2's two rainbow side bands are elastic material. Candidate Y was
; preserved in bestDistance before the independent-axis collision tests.
; Zone 2-3 additionally has two centered elastic platforms; stageOffset keeps
; the rejected candidate X until both independent-axis tests are complete.
IsChapter2ElasticContact:
        lda currentStage
        cmp #1
        bne .checkZone3Elastic
        lda bestDistance
        cmp #88
        bcs .ordinaryMaterial
        clc
        adc #ASTRONAUT_H
        cmp #73
        bcc .ordinaryMaterial
        sec
        rts
.checkZone3Elastic:
        cmp #2
        bne .ordinaryMaterial
        lda stageOffset
        clc
        adc #ASTRONAUT_W
        cmp #65                ; candidate right edge must reach platform X=64
        bcc .ordinaryMaterial
        lda stageOffset
        cmp #96                ; candidate left edge must precede platform X=96
        bcs .ordinaryMaterial
        lda bestDistance
        cmp #72
        bcs .checkLowerElasticPlatform
        clc
        adc #ASTRONAUT_H
        cmp #65                ; candidate bottom edge must reach platform Y=64
        bcc .ordinaryMaterial
        sec
        rts
.checkLowerElasticPlatform:
        cmp #112
        bcs .ordinaryMaterial
        clc
        adc #ASTRONAUT_H
        cmp #105               ; candidate bottom edge must reach platform Y=104
        bcc .ordinaryMaterial
        sec
        rts
.ordinaryMaterial:
...
; Double signed 8.8 horizontal speed, capped at +/-1.75 px/frame.
DoubleHorizontalVelocityCapped:
        asl velXLo
        rol velXHi
        lda velXHi
        bmi .checkNegativeElasticSpeed
        cmp #1
        bcc .elasticSpeedReady
        bne .capPositiveElasticSpeed
        lda velXLo
        cmp #$C1
        bcc .elasticSpeedReady
.capPositiveElasticSpeed:
        lda #$01
        sta velXHi
        lda #$C0
        sta velXLo
        rts
.checkNegativeElasticSpeed:
        cmp #$FE
        bcc .capNegativeElasticSpeed
        bne .elasticSpeedReady
        lda velXLo
        cmp #$40
        bcs .elasticSpeedReady
.capNegativeElasticSpeed:
        lda #$FE
        sta velXHi
        lda #$40
        sta velXLo
...
.checkSoftwareSides:
        ...
        bcc .softwareHit
        IFCONST CHAPTER2_ZONE3_LAB
        ; Zone 2-3 adds three centered PF2 platforms spanning X=64..95.
        ; Test their exact rectangle after the independent side-wall test.
        ldy currentStage
        cpy #2
        bne .softwareClear
        lda tomXHi
...
StageSpawnY:
        IFCONST CHAPTER2_ZONE2_LAB
        byte SPAWN_Y,146,STAGE3_SPAWN_Y,STAGE4_SPAWN_Y,STAGE5_SPAWN_Y
        ELSE
        IFCONST CHAPTER2_ZONE1_LAB
...
StageNextByStage:
        IFCONST CHAPTER2_ZONE3_LAB
        byte 1,2,NO_NEXT_STAGE,4,NO_NEXT_STAGE
        ELSE
        IFCONST CHAPTER2_ZONE2_LAB
...
Stage2LeftInsetByBand:
        IFCONST CHAPTER2_ZONE2_LAB
        byte 80,28,20,24,36,16,48,40,16,40,36
        byte 16,28,36,24,20,32,48,44,28,28,80
        ELSE
...
Stage2RightInsetByBand:
        IFCONST CHAPTER2_ZONE2_LAB
        byte 80,24,24,40,48,16,24,20,24,36,28
        byte 32,40,16,48,44,44,40,28,16,32,80
        ELSE
...
Stage3RightInsetByBand:
        IFCONST CHAPTER2_ZONE3_LAB
        byte 80,28,32,36,28,24,20,16,16,16,16
        byte 16,16,16,16,16,20,24,28,32,36,80
        ELSE
...
Bank2Boot:
        ...
        jmp $F006
        IFCONST CHAPTER2_ZONE3_LAB
; Unreachable lab-build signature.  Stella keys cartridge-specific debugger
; state by ROM identity; keeping a distinct byte here prevents an interrupted
; 2-3 debug session from being restored over the next clean playtest.
Zone23BuildSignature:
        byte $23
        ENDIF
        ORG $2FFA
        RORG $FFFA
...
Stage3VisorEnable:
        ...
        RORG $F600
        IFCONST CHAPTER2_ZONE3_LAB
        include "thursdays-child-stage3-zone2-3-objects.inc"
        ELSE
...
Stage3TerrainColor:
        IFCONST CHAPTER2_ZONE3_LAB
        ; Chapter Two's yellow divider and orange cavern frame surround three
        ; medium center platforms. The upper/lower rainbow ramps are elastic;
        ; the darker middle platform is ordinary rock.
...
Stage3StarOddControl:
        ...
        tay
        lda Stage3ElasticFlowColors,y
        jmp Stage3PlatformColorReady
.platformRestore:
        lda #$34               ; rows 72 and 112 both return to this orange
        jmp Stage3PlatformColorReady
        ELSE
        bmi .restore
        eor bestBeacon
...
Stage2TerrainColor:
        ...
        ds 8,$32
        ds 8,$34
        ; Two side-only collision bands read as elastic material because PF2
        ; remains empty through their center. Their opposing rainbow ramps
        ; distinguish them immediately from the orange rock teeth.
        byte $4C,$3E,$2E,$CE,$AE,$8E,$6E,$5E
        byte $5E,$6E,$8E,$AE,$CE,$2E,$3E,$4C
        ds 8,$3A
        ds 8,$3C
...
Stage2StarControl:
        IFCONST CHAPTER2_ZONE2_LAB
        ; On each odd row, pre-stage the following even row's flowing shelf
        ; color. $80 restores the ordinary orange color before row 88.
        ds 71,0
...
.shelfRestore:
        lda #$3A               ; Stage2TerrainColor row 88
        jmp Stage2ShelfColorReady
        ELSE
        bmi .restore
        eor bestBeacon
...
Stage2StarOddSync:
        ...
        IFCONST CHAPTER2_ZONE2_LAB
Stage2ShelfColorReady:
        ; Hold the prepared color in A across WSYNC, then commit it during
        ; horizontal blank. Jump past the ordinary table load so neither side
        ; shelf can change color while visible pixels are being drawn.
        sta WSYNC
        sta COLUPF
        inx
        ; Match the ordinary kernel's cycle-25 left PF1 slot exactly. Both an
        ; early and a late reflected-playfield write create visible fragments.
        nop
        nop
        nop
        nop
        nop
        jmp Stage2RoomGeometry
        ENDIF
