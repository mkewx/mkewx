...
- The discarded long center platforms are prohibited: each divided the room
  ...
  Major Tom.
- Zone 2-2 uses asymmetric side-attached orange teeth around one continuous
  cavity. Center islands were rejected after they intersected satellite orbit
  envelopes and made Major Tom appear displaced instead of circling. The
  rainbow effect remains on the elastic floor; an interior rainbow bumper is
  deferred until a later zone can introduce it without obstructing any orbit.
- Major Tom launches naturally from `(68,144)` with velocity `(+$0060,+$0060)`.
